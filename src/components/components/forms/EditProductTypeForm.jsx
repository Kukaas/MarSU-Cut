...
const EditProductTypeForm = ({ onClose, onSuccess, selectedProductType }) => {
  ...
  const form = useForm({
    ...
    defaultValues: {
      level: selectedProductType.level,
      productType: selectedProductType.productType,
      size: selectedProductType.size,
      rawMaterialsUsed: selectedProductType.rawMaterialsUsed.map(
        (material) => ({
    ...
    const fetchRawMaterialTypes = async () => {
      try {
        ...
        if (res.status === 200) {
          ...
          setUnits(uniqueUnits.filter(Boolean)); // Filter out any empty strings
        }
      } catch (error) {
        console.error(error);
      }
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAllSizes(sizesData.filter((size) => size.size)); // Filter out any sizes with empty strings
      } catch (error) {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        form.reset();
        onSuccess(res.data.updatedProductType);
        onClose();
      }
    } catch (error) {
...
EditProductTypeForm.propTypes = {
  ...
  selectedProductType: PropTypes.object.isRequired,
};
